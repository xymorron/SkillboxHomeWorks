...
(function ($) {
    ...
    $(function () {
        ...
        var Card = function () {
            var bookTemplate = function (book) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="' + book.slug + '">' + '       <img src="' + book.image + '">\n' + '</a>' +
                    ...
                    (book.discountPrice ? '<span class="Card-priceOld">₽' + book.price + '</span>\n' : '') + '                       <span class="Card-price">₽' + (book.discountPrice ? book.discountPrice : book.price) + '</span>\n' + '                </div>\n' + '              </div>\n' + '            </div>';
            }
            return {
                init: function () {
            ...
            return {
                init: function () {
                    ...
                    $btnSend.on('click', function (e) {
                        ...
                        if ($this.data('sendstatus') === 'ARCHIVED' && $this.data('check')) {
                            ...
                            // Login().postData('/changeBookStatus/' + $this.data('bookid'), {
                                ...
                                if (result.result) {
                                    ...
                                    // задел для пересчета значений отложенного и корзины при изменении статуса кники
                                    // альтернативный вариант - запрос количества через api
                                    // if (status === 'KEPT') {
                                    //     var keptAmount = parseInt($('.Kept-amount').text());
                                    //     console.log('keptAmount=' + keptAmount);
                                    //     keptAmount += 1;
                                    //     console.log('increased keptAmount=' + keptAmount);
                                    //
                                    //     $('.Kept-amount').text(keptAmount);
                                    // }
                                    ...
                                    if ($cart.length) {
                                        ...
                                        $cart.find('.Cart-product').each(function () {
                                            var $this = $(this);
                                            var currentPrice = 0, currentOldPrice = 0;
                                            currentPrice = parseFloat($this.find('.Cart-price:not(.Cart-price_old)').text());
                                            currentOldPrice = parseFloat($this.find('.Cart-price_old').text());
                                            price += currentPrice;
                                            priceOld += isNaN(currentOldPrice) ? currentPrice : currentOldPrice;
                        ...
                        // полное обновление страницы после отправки изменений
                        // setTimeout(function(){
        ...
        var Spoiler = function () {
            var $HideBlock = $('.Spoiler');
            var $SpoilerSmall = $('.Spoiler-small');
            var $trigger = $HideBlock.find('.Spoiler-trigger');
            $HideBlock.addClass('Spoiler_CLOSE');
            return {
                init: function () {
                    $trigger.on('click', function (e) {
                        ...
                        if ($parent.hasClass('Spoiler_CLOSE')) {
                            $parent.removeClass('Spoiler_CLOSE');
                            $SpoilerSmall.addClass('Spoiler-hide');
                            $(window).scrollTop(scroll);
                        } else {
                            $parent.addClass('Spoiler_CLOSE');
                            $SpoilerSmall.removeClass('Spoiler-hide');
                            $(window).scrollTop(scroll);
                        }
                    });
                }
